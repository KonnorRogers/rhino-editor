...
function parseContentTypeFromElement (element: HTMLElement) {
  return (
    ...
    JSON.parse(element.getAttribute("data-trix-attachment") || "{}").contentType ||
    "application/octet-stream"
  )
}

const canParsePreviewAttachment = (node: HTMLElement | string, shouldPreview: boolean) => {
  ...
  parseHTML() {
    return [
      ...
      // There is no <figcaption> here.
      {
        ...
        getAttrs: (node) => {
          const isValid = canParsePreviewAttachment(node, this.options.previewable);
        ...
        if (typeof getPos === "function") {
          ...
          view.dispatch(
            view.state.tr.setSelection(
              TextSelection.create(view.state.doc, getPos() + 1),
            ),
          );
        }
      }
